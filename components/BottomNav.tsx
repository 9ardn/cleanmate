...
export type NavKey = 'home' | 'inbox' | 'tasks' | 'stats' | 'mypage';

interface BottomNavProps {
  state: RoomState;
  inboxBadge?: number;
  /** For single-page app mode: pass current key and callback */
  activeKey?: NavKey;
...
export function BottomNav({ state, inboxBadge = 0, activeKey, onNavigate }: BottomNavProps) {
  ...
  return (
    ...
    >
      ...
      >
        {NAV_ITEMS.map((nav) => {
          const isActive = onNavigate ? activeKey === nav.key : nav.match(pathname);
          const Icon = nav.icon;
          const badge = nav.key === 'inbox' ? inboxBadge : 0;

          const content = (
            <>
              <Icon size={17} style={{ color: isActive ? t.accent : t.text, opacity: isActive ? 1 : 0.5 }} />
              <span
                className="text-[9px] mt-1 font-bold"
                style={{ color: isActive ? t.accent : t.text, opacity: isActive ? 1 : 0.5 }}
              >
                {nav.label}
              </span>
              {badge > 0 && (
          ...
          const className = 'relative flex-1 flex flex-col items-center py-2 px-1.5 rounded-xl transition-all active:scale-90';
          const style = { background: isActive ? `${t.accent}15` : 'transparent' };

          if (onNavigate) {
            return (
              <button key={nav.key} onClick={() => onNavigate(nav.key)} className={className} style={style}>
                {content}
              </button>
            );
          }
          return (
            <Link key={nav.key} href={nav.href} className={className} style={style}>
              {content}
            </Link>
          );
        })}
